...
func _update_time_label() -> void:
	time_label.text = "Время: " + World.get_current_time_string()
...
func _refresh_characters_list() -> void:
	...
	for c in characters_in_selected_building:
		characters_list.add_item("%s — %s" % [c.full_name(), _action_text(c.current_action)])
...
func _format_character_details(c: CharacterData) -> String:
	var lines: Array[String] = [
		...
		"Действие: %s" % _action_text(c.current_action),
		"",
