...
@export var cafe_hunger_relief: float = 45.0
...
@export var shop_fun_gain: float = 6.0
...
## конкретному гостю), и у официантов (обслуживает конкретный столик).
@export var staff_serve_chance: float = 0.95
...
static func get_definitions() -> Array[Dictionary]:
	return [
		...
		{"label": "Снижение голода едой дома", "property": "eat_hunger_relief", "min": 5.0, "max": 60.0, "step": 1.0},
		{"label": "Снижение голода едой в кафе", "property": "cafe_hunger_relief", "min": 5.0, "max": 80.0, "step": 1.0},
